...
body { font-family: var(--ff); background: var(--bg); color: var(--t1); min-height: 100vh; display: flex; flex-direction: column; -webkit-font-smoothing: antialiased; }
...
.nav-links a { font-size: 0.875rem; font-weight: 500; color: var(--t2); padding: 6px 13px; border-radius: 8px; transition: all 0.18s; text-decoration: none; }
...
.nav-cta:hover { background: var(--nv2) !important; transform: translateY(-1px); box-shadow: var(--sh-md) !important; color: #fff !important; }
...
.back-link {
  ...
  margin-bottom: 10px; transition: color 0.18s; text-decoration: none;
}
...
.btn-sm {
  ...
  background: var(--rs-dim); color: var(--rs); border: 1px solid rgba(168,88,88,0.3);
  cursor: pointer;
}
.btn-sm:hover { background: var(--rs); color: #fff; }
...
<header class="navbar">
  ...
  <nav class="nav-links">
    ...
    <a href="/participant/profile" class="nav-cta">My Profile</a>
...
<div class="page">
  ...
  <!-- MESSAGES (simplified for brevity - same as original) -->
  <c:if test="${success == 'memberInvited'}">
    <div class="msg success"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>Invitation sent to participant.</div>
  </c:if>
  <c:if test="${success == 'externalInvited'}">
    <div class="msg success"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>External invite saved successfully.</div>
  </c:if>
  <c:if test="${success == 'memberRemoved'}">
    <div class="msg success"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>Member removed from team successfully.</div>
  </c:if>
  <c:if test="${success == 'teamCreated'}">
    <div class="msg success"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>Team created successfully.</div>
  </c:if>
  <c:if test="${success == 'teamJoined'}">
    <div class="msg success"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>You joined the team successfully.</div>
  </c:if>
  <c:if test="${success == 'inviteAccepted'}">
    <div class="msg success"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>Invitation accepted.</div>
  </c:if>
  <c:if test="${error == 'teamFull'}">
    <div class="msg error"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>Your team is full.</div>
  </c:if>
  <c:if test="${error == 'alreadyInHackathon'}">
    <div class="msg error"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>Participant already in a team.</div>
  </c:if>
  <c:if test="${error == 'notLeader'}">
    <div class="msg error"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>Only team leader can do this.</div>
  </c:if>
  ...
  <div class="grid">
    ...
    <c:if test="${not hasTeam}">
      ...
      <c:if test="${not empty pendingInvite}">
        <div class="card grid-full">
          <div class="card-head">
            <span class="card-head-title"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.41 2 2 0 0 1 3.6 1.24h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7.91 8.96a16 16 0 0 0 6 6l.92-.92a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 21.73 16.92z"/></svg>Pending Invitation</span>
            ...
            <div class="invite-banner">
              ...
              <div class="btn-row" style="margin-top:0">
                <form action="/participant/hackathon/${hackathon.hackathonId}/team/invite/${pendingInvite.hackathonTeamInviteId}/accept" method="post">
                  <button type="submit" class="btn btn-primary" style="margin-top:0; background:var(--gd); box-shadow:0 2px 8px var(--gd-dim)">✓ Accept</button>
                </form>
                <form action="/participant/hackathon/${hackathon.hackathonId}/team/invite/${pendingInvite.hackathonTeamInviteId}/reject" method="post">
                  <button type="submit" class="btn btn-danger" style="margin-top:0">✗ Reject</button>
                </form>
              </div>
            </div>
          </div>
      ...
      <!-- CREATE TEAM -->
      <div class="card">
        <div class="card-head"><span class="card-head-title"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Create Team</span></div>
        <div class="card-body">
          <form action="/participant/hackathon/${hackathon.hackathonId}/team/create" method="post">
            ...
            <button type="submit" class="btn btn-navy">+ Create Team</button>
          </form>
        </div>
      </div>
      ...
      <!-- JOIN TEAM -->
      <div class="card">
        <div class="card-head"><span class="card-head-title"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>Join Existing Team</span></div>
        <div class="card-body">
          <c:choose>
            <c:when test="${empty availableTeams}">
              <div class="empty-state"><div class="empty-ico"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg></div><p>No open teams available.</p></div>
            </c:when>
            <c:otherwise>
              <form action="/participant/hackathon/${hackathon.hackathonId}/team/join-existing" method="post">
                <div class="form-group">
                  ...
                  <select name="joinTeamId" class="form-select" required>
                    ...
                    <c:forEach items="${availableTeams}" var="t"><option value="${t.hackathonTeamId}">${t.teamName}</option></c:forEach>
                  </select>
                </div>
    ...
    <c:if test="${hasTeam}">
      ...
      <!-- TEAM MEMBERS TABLE -->
      <div class="card">
        <div class="card-head"><span class="card-head-title"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>Team Members</span><span class="card-head-badge">${teamSizeCount} / ${teamMaxSize}</span></div>
        <div class="card-body">
          <table class="table">
            <thead><tr><th>#</th><th>Name</th><th>Email</th><th>Role</th><th>Action</th></thead>
            <tbody>
              <c:forEach items="${teamMembers}" var="m" varStatus="i">
                <tr>
                  <td style="color:var(--t3);">${i.count}</td>
                  ...
                  <td>
                    <c:choose><c:when test="${m.memberId == team.teamLeaderId}"><span class="role-chip role-leader">Leader</span></c:when><c:otherwise><span class="role-chip role-member">${m.roleTitle}</span></c:otherwise></c:choose>
                  </td>
                  <td>
                    <c:if test="${isTeamLeader and m.memberId != team.teamLeaderId}">
                      <form action="/participant/hackathon/${hackathon.hackathonId}/team/remove-member" method="post" style="display:inline">
                        ...
                        <button type="submit" class="btn-sm" onclick="return confirm('Remove this member?')">Remove</button>
                      </form>
                    </c:if>
                  </td>
                 </tr>
      ...
      <div style="display:flex; flex-direction:column; gap:20px;">
        ...
        <c:if test="${isTeamLeader and inviteAllowed}">
          <div class="card">
            <div class="card-head"><span class="card-head-title"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>Invite Participant</span></div>
            <div class="card-body">
              <form action="/participant/hackathon/${hackathon.hackathonId}/team/invite-member" method="post">
                <div class="form-group"><label class="form-label">Select Participant</label><select name="invitedUserId" class="form-select" required><option value="">Choose participant...</option><c:forEach items="${participantUsers}" var="u"><option value="${u.userId}">${u.firstName} ${u.lastName} — ${u.email}</option></c:forEach></select></div>
          ...
          <div class="card">
            <div class="card-head"><span class="card-head-title"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>Invite External User</span></div>
            <div class="card-body">
              <form action="/participant/hackathon/${hackathon.hackathonId}/team/invite-external" method="post">
                <div class="form-group"><label class="form-label">External Email</label><input type="email" name="externalEmail" class="form-input" placeholder="name@example.com" required></div>
                <div class="form-group"><label class="form-label">Role Title</label><input type="text" name="roleTitle" class="form-input" placeholder="e.g. MEMBER"></div>
                <button type="submit" class="btn btn-primary">Send External Invite</button>
              </form>
            </div>
          </div>
        </c:if>
        ...
        <c:if test="${isTeamLeader and not inviteAllowed}">
          <div class="card"><div class="card-head"><span class="card-head-title"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>Invite Members</span></div><div class="card-body"><div class="disabled-notice"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>Invitations disabled after registration ends.</div></div></div>
        </c:if>

        <c:if test="${not isTeamLeader}">
          <div class="card"><div class="card-head"><span class="card-head-title"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>Invite Members</span></div><div class="card-body"><div class="disabled-notice"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>Only team leader can send invites.</div></div></div>
        </c:if>
      ...
      <!-- INVITE ACTIVITY -->
      <div class="card grid-full">
        <div class="card-head"><span class="card-head-title"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>Invite Activity</span></div>
        <div class="card-body">
          <table class="table"><thead><tr><th>Type</th><th>Invitee</th><th>Status</th><th>Sent On</th></tr></thead>
          <tbody>
            <c:if test="${empty inviteList}"><tr><td colspan="4"><div class="empty-state" style="padding:24px 0"><div class="empty-ico"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg></div><p>No invites sent yet.</p></div></td></tr></c:if>
            <c:forEach items="${inviteList}" var="i"><tr><td><span class="type-chip">${i.inviteType}</span></td><td style="color:var(--t2)"><c:choose><c:when test="${not empty i.invitedEmail}">${i.invitedEmail}</c:when><c:otherwise>User #${i.invitedUserId}</c:otherwise></c:choose></td><td><c:choose><c:when test="${i.inviteStatus == 'PENDING'}"><span class="status-chip status-pending">PENDING</span></c:when><c:when test="${i.inviteStatus == 'ACCEPTED'}"><span class="status-chip status-accepted">ACCEPTED</span></c:when><c:otherwise><span class="status-chip status-rejected">REJECTED</span></c:otherwise></c:choose></td><td style="color:var(--t3); font-size:0.82rem">${i.createdAt}</td></tr></c:forEach>
          </tbody></table>
        </div>
      </div>
